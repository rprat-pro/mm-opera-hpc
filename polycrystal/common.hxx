...
/**
 * @brief Parses common command-line parameters and populates the TestParameters
 * structure.
 *
 * This function parses common command-line parameters using the provided `args`
 * object and populates the `TestParameters` structure `p` with the parsed
 * values. The common parameters include options for configuring various aspects
 * of the test.
 *
 * @param args An instance of the `mfem::OptionsParser` class for parsing
 * command-line arguments.
 * @param p    A reference to the `TestParameters` structure where parsed values
 * will be stored.
 *
 * @note The `TestParameters` structure should be defined prior to calling this
 * function, as it will be modified to contain the parsed values.
 *
 * @see TestParameters
 ...
 * to a Problem instance of the specified type. Post-processing can include
 * tasks such as generating and saving visualizations, stress analysis, and
 * more.
 *
 * @tparam Problem              The type of the Problem instance to which
 * post-processing will be added.
 *
 * @param p                     A reference to the Problem instance to which
 * post-processing will be added.
 * @param msg                   Paraview file name (results).
 * @param stress_output_name    The name or identifier for stress-related output
 * data.
 * @param paraview_output_name  Paraview file with integration point value.
 *
 * @note This function allows you to customize and configure various
 * post-processing options specific to the given Problem type. Please refer to
 * the documentation of the Problem class for details on available
 * post-processing options and their meanings.
 */
template <typename Problem>
void add_post_processings(Problem &p,
                          std::string msg,
                          std::string stress_output_name,
  ...
  p.addPostProcessing(
      ...
      {{"OutputFileName", paraview_output_name}, {"Results", {"Stress"}}});
}
 ...
 */
template <typename Problem>
void add_post_processings_2(Problem &p,
                            std::string msg,
                            std::string stress_output_name) {
  p.addPostProcessing("ParaviewExportResults", {{"OutputFileName", msg}});
  p.addPostProcessing("MeanThermodynamicForces",
                      {{"OutputFileName", stress_output_name}});
  p.addPostProcessing("ParaviewExportIntegrationPointResultsAtNodes",
                      {{"OutputFileName", msg + "IntegrationPointOutput"},
                       {"Results", {"FirstPiolaKirchhoffStress"}}});
}  // end timer add_postprocessing_and_outputs
/**
 * @brief Executes post-processing steps for a given Problem instance within a
 * specified time range.
 *
 * @tparam Problem  The type of the Problem instance for which post-processing
 * will be executed.
 *
 * @param p         A reference to the Problem instance for which
 ...
 *
 * @note This function is responsible for executing the defined post-processing
 * steps within the specified time range.   The specific post-processing steps
 * and their configurations should have been previously added to the Problem
 *       instance using the `add_post_processings` function or other appropriate
 * methods.
 */
template <typename Problem>
 ...
 *
 * @note This function is responsible for solving the Problem instance within
 * the specified time range using the provided time step. It also checks the
 * status of the solver and aborts if the solve operation fails.
 */
template <typename Problem>
void run_solve(Problem &p, double start, double dt) {
  CatchTimeSection("Solve");
  mfem_mgis::Profiler::Utils::Message("Solving problem from: ", start,
                                      " to: ", start + dt);
  // solving the problem
