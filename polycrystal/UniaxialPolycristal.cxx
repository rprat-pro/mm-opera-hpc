...
void setup_properties(const TestParameters &p,
  ...
  std::vector<std::array<mfem_mgis::real, 3u>> vectors =
      readVectorsFromFile(p.vect_file);
...
void simulation(Problem &problem, double start, double end, double dt,
  ...
  // ===========================
  const double C11 =
      young1 * (poisson12 - 1) / (2 * poisson12 * poisson12 + poisson12 - 1);
  const double C12 =
      -(poisson12 * young1) / (2 * poisson12 * poisson12 + poisson12 - 1);
  ...
  problem.setMacroscopicGradientsEvolution(
      [&FXX, &FYY, &FZZ, &SIGXX, &SIGYY, Eeff, nueff](const double t) {
  ...
  for (int i = 0; i < nStep; i++) {
    ...
    while ((!converged) && (itFP <= maxitFP)) {
      // compute correction
      double Fcorr[2];
      Fcorr[0] = ((nueff * nueff - 1) * SIGXX) / Eeff +
                 (nueff * (nueff + 1) * SIGYY) / Eeff;
      Fcorr[1] = ((nueff * nueff - 1) * SIGYY) / Eeff +
                 (nueff * (nueff + 1) * SIGXX) / Eeff;
      ...
      SIGYY = 0.;
      for (const auto &mi : problem.getAssignedMaterialsIdentifiers()) {
        SIGXX += tf_integrals[mi][0];
        SIGYY += tf_integrals[mi][1];
      }
      SIG_local[0] = SIGXX;
      SIG_local[1] = SIGYY;
      MPI_Allreduce(SIG_local, SIG_global, 2, MPI_DOUBLE, MPI_SUM,
                    MPI_COMM_WORLD);
      SIGXX = SIG_global[0] / vol;
      SIGYY = SIG_global[1] / vol;
    ...
    if (mfem_mgis::getMPIrank() == 0) {
      ...
      if (fichier.is_open()) {
        fichier << (i + 1) * dt << " " << FXX << " " << FYY << " " << FZZ
                << "\n";
        fichier.close();
        std::cout << "Les valeurs ont été écrites dans le fichier."
                  << std::endl;
      } else {
