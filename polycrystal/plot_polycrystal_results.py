...
data_res = np.loadtxt("uniaxial-polycrystal.res")  # par défaut espace = séparateur

x_res = data_res[:, 0]  # 1ère colonne
y_res = data_res[:, 6]  # 7ème colonne (index 6 car Python commence à 0)

# --- Lecture du fichier CSV ---
data_csv = pd.read_csv("results/polycrystal_cast3m.csv", sep=";")
...
plt.title("Polycrystal test case with 5 crystals")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig("plot_polycrystal.png")
