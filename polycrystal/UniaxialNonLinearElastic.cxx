  ...
  for (int i = 0; i < nStep; i++) {
    ...
    while ((abs(oldRes - newRes) > tolFP) && (itFP <= maxitFP)) {
      problem.setMacroscopicGradientsEvolution(
          [&FXX, &FYY, &FZZ, &SIGXX, &SIGYY, young](const double t) {
      ...
      auto [tf_integrals, volumes] = computeMeanThermodynamicForcesValues<true>(
...
int main(int argc, char* argv[]) {
  ...
  if (use_post_processing)
    add_post_processings_2(problem, "OutputFile-Uniaxial-NonLinearElastic",
                           "avgStressNonLinearElastic");
