...
void setup_properties(const TestParameters &p,
  ...
  auto rot = [](std::array<mfem_mgis::real, 3u> u,
    ...
    std::array<mfem_mgis::real, 9u> ret = {
...
static void setLinearSolver(Problem &p,
  ...
  // solverParameters.insert(mfem_mgis::Parameters{{"VerbosityLevel",
  // verbosity}});
  // solverParameters.insert(mfem_mgis::Parameters{{"MaximumNumberOfIterations",
  // defaultMaxNumOfIt}});
  // // solverParameters.insert(mfem_mgis::Parameters{{"AbsoluteTolerance",
  // Tol}});
  // // solverParameters.insert(mfem_mgis::Parameters{{"RelativeTolerance",
  // Tol}}); solverParameters.insert(mfem_mgis::Parameters{{"Tolerance", Tol}});
  ...
  // {"Options",options}};
  // // auto preconditionner = mfem_mgis::Parameters{{"Name","HypreBoomerAMG"},
  // {"Options",options}};
...
void simulation(
  ...
  for (int i = 0; i < nStep; i++) {
    ...
    double EXX = -((poisson12 + 1) * poisson13 * young3) /
                 (young1 - poisson13 * poisson13 * young3) * EZZ;
    // double EYY =
    // (poisson12*poisson12+poisson13)*young3/(young1-poisson12*poisson12*young3)*EZZ;
    ...
    while ((abs(oldRes - newRes) > tolFP) && (itFP <= maxitFP)) {
      ...
      problem.setMacroscopicGradientsEvolution([&EXX, EZZ, &SIGXX, young1,
                                                young3, poisson13,
                                                poisson12](const double t) {
        auto ret = std::vector<mfem_mgis::real>(6, mfem_mgis::real{0});
        ret[2] = EZZ;
        ret[0] =
            EXX +
            ((1 + poisson12) *
             (2 * poisson13 * poisson13 * young3 + young1 * (poisson12 - 1))) /
                (young1 * (young1 - poisson13 * poisson13 * young3)) * SIGXX;
        EXX = ret[0];
      ...
      for (const auto &mi : problem.getAssignedMaterialsIdentifiers()) {
        // volumes[mi] ;
        mfem_mgis::Profiler::Utils::Message("tf_integrals[mi] = ",
                                            tf_integrals[mi][0]);
        SIGXX += tf_integrals[mi][0] / vol;
      }

      SIG_local[0] = SIGXX;
      MPI_Allreduce(SIG_local, SIG_global, 1, MPI_DOUBLE, MPI_SUM,
                    MPI_COMM_WORLD);
      SIGXX = SIG_global[0];

      oldRes = newRes;
      newRes = std::sqrt(SIGXX * SIGXX);
      mfem_mgis::Profiler::Utils::Message("FIXED POINT iteration", itFP,
                                          ": |res| =", abs(oldRes - newRes));
...
int main(int argc, char *argv[]) {
  ...
  if (use_post_processing)
    add_post_processings(problem, "OutputFile-Uniaxial-elastic-ortho",
                         "avgStressElasticOrtho",
                         "UniaxialElasticOrthoTestIntegrationPointOutput");
