  ...
  for (mfem_mgis::size_type &bid : target_boundary_attribute) {
    ...
    for (int i = 0; i < mesh->GetNBE(); i++) {
      if (mesh->GetBdrAttribute(i) == bid) {
        ...
        // This transformation maps from reference space to physical space for the boundary face
        mfem::ElementTransformation *bdr_tr =
            mesh->GetBdrElementTransformation(i);

        // Get face-element transformations. 
        // This provides information about the domain element adjacent to this boundary face. 
        // For boundary faces, Elem1 is the interior element
        mfem::FaceElementTransformations *face_elem_tr =
            mesh->GetFaceElementTransformations(i, true); // NB true selects the boundary elements
        ...
        int domain_elem_idx =
            face_elem_tr->Elem1No;

        // Check if the parent domain element belongs to the current material.
        // If not, its quadrature points are not managed by this 'pqs' and 'm'.
        if (fespace_mgis.GetAttribute(domain_elem_idx) != mid) {
          continue;
        }

        // Get information for the parent domain element to build an integration rule 
        // we need the finite element type and the polynomial degree
        const auto &domain_fe_mgis = *(fespace_mgis.GetFE(domain_elem_idx));
        auto &domain_elem_tr =
            *(fespace_mgis.GetElementTransformation(domain_elem_idx));

        // Get the integration rule (set of quadrature points) for the domain element
        // as defined by the partial quadrature space.
        ...
        // Jacobian order of the transformation and the order of the domain FE.
        ...
        // Pre-calculate and store the physical coordinates of all quadrature points
        // within the parent domain element.
        ...
        for (int dq_idx = 0; dq_idx < domain_qps_ir_mgis.GetNPoints();
             ++dq_idx) {
          // Get the reference coordinates of the current domain quadrature point.
        ...
        for (int bg = 0; bg < bdr_face_ir.GetNPoints(); ++bg) {
          // Get the reference coordinates of the current boundary face quadrature point.
          const mfem::IntegrationPoint &bdr_face_ip_ref =
              bdr_face_ir.IntPoint(bg);

          // Transform the boundary face quadrature point to physical coordinates.
          ...
          // Find the closest domain quadrature point (from the parent element)
          // to the current boundary face quadrature point.
          ...
          if (closest_domain_qp_g_idx != -1) {
            // The offset is (domain_elem_offset + local_qp_index) * number_of_stress_components.
            ...
            // Compute the eigenvalues of the stress tensor -> principal stress
            ...
            std::cout << "      Warning: Could not find a closest domain QP\n";
          }
        }
      }
    }
...
std::vector<StressValueAndId> accessBoundaryClosestQPData(
  ...
  // Check if the finite element discretization describes a parallel computation.
