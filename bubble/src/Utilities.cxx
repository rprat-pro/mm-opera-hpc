...
namespace opera_hpc {
  ...
  getPointsandStressAboveStressThresholdImplementation(
    ...
    const auto *stress_values = m.s1.thermodynamic_forces.data();
    //std::vector<std::array<mfem_mgis::real, 3u>> local_locations;
    std::vector<std::pair<mfem_mgis::real, std::array<mfem_mgis::real, 3u>>> collected_points_with_svp;
    // loop over the elements
    mfem::Vector tmp;
    for (mfem_mgis::size_type i = 0; i != fespace.GetNE(); ++i) {
      if (fespace.GetAttribute(i) != mid) {
    ...
    std::vector<std::pair<mfem_mgis::real, std::array<mfem_mgis::real, 3u>>> locations_and_svp_values;
    ...
    if constexpr (parallel) {
      ...
      for (int idx = 0; idx != lsize; ++idx) {
        local_items_tmp[values_per_item * idx] = collected_points_with_svp[idx].first;
        local_items_tmp[values_per_item * idx + 1] = collected_points_with_svp[idx].second[0];
        local_items_tmp[values_per_item * idx + 2] = collected_points_with_svp[idx].second[1];
        local_items_tmp[values_per_item * idx + 3] = collected_points_with_svp[idx].second[2];
        //std::cout << local_locations[idx][0] << "\t" << local_locations[idx][1] << "\t" << local_locations[idx][2] << std::endl;
      }

      std::vector<int> item_counts_per_proc(nprocs);
      MPI_Allgather(&lsize, 1, MPI_INT, item_counts_per_proc.data(), 1, MPI_INT, MPI_COMM_WORLD);
      ...
      if (nprocs > 0) {
        ...
        for (int i = 1; i < nprocs; ++i) {
          displacements_items[i] = displacements_items[i - 1] + item_counts_per_proc[i - 1];
